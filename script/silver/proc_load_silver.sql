	...
	BEGIN TRY
		...
		SELECT
			...
			CASE
				...
				ELSE 'n/a'
			END AS cst_maritial_status, --Normalize maritial status values to readable format
			CASE
		...
		SELECT 
		prd_id,
		REPLACE(SUBSTRING(prd_key, 1, 5), '-', '_') AS cat_id, --Extract catogery id.
		...
		--DATA TRANSFORMATION FOR BRONZE TABLE INSERTING INTO SILVER TABLE.
		SELECT
		sls_ord_num,
		sls_prd_key,
		sls_cust_id,
		...
		CASE WHEN sls_sales IS NULL OR sls_sales <=0 OR sls_sales != sls_quantity * ABS(sls_price)
			THEN sls_quantity * ABS(sls_price) --Handling missing data, invalid data
		ELSE sls_sales
		END AS sls_sales,
		...
		PRINT '------------------------------------------------';
		--Data cleansing erp_cust_az12 table.
		SET @start_time = GETDATE();
		...
		SELECT
		CASE WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid,4,LEN(cid)) --Remove invalid values prifixes.
			 ELSE cid
		END AS cid,
		...
		CASE WHEN UPPER(TRIM(gen)) IN ('F', 'FEMALE') THEN 'Female'
			 ...
			 ELSE 'n/a'
		END AS gen --Normalizing gender vsalues and handle unknown cases.
		...
		--Data cleansing of silver table erp_loc_a101
		SET @start_time = GETDATE();
		...
		--Data cleansing if silver table erp_px_cat_g1v2.
		SET @start_time = GETDATE();
